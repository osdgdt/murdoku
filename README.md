...
App web statica, senza build: HTML/CSS/JS con ES modules. Editor, gioco libero, stampa ed export/import restano interamente senza backend (tutto in `localStorage`); solo la **modalità campagna** (sotto) usa Firebase per account e progresso condiviso tra dispositivi — il resto del sito funziona esattamente come sempre, senza bisogno di alcun accesso.
...
7. **Salva**, **Esporta JSON** per condividere il file, o **Stampa** per una versione cartacea. Il pulsante **🗑 Elimina** rimuove definitivamente il caso corrente (con richiesta di conferma); **📋 Duplica** crea una copia indipendente del caso (utile come base per un nuovo puzzle) senza toccare l'originale.
   ...
   Per condividere un caso con un link diretto (invece di far scaricare e importare manualmente il JSON esportato) basta mettere il file `.murdoku.json` online accanto al sito e mandare `player.html?import=<url-del-file>` — apre il caso e comincia subito a giocarlo. Rivisitare lo stesso link non cancella mai i progressi già fatti.
...
### Giocare
Apri **Gioca**, scegli un caso salvato (gli elenchi si possono ordinare per aggiornamento, titolo o difficoltà; i casi già risolti hanno un segno ✓), e seleziona un personaggio (o lo strumento X, o la gomma) dalla toolbar. Sulla griglia, il gesto decide cosa succede: un **clic/tocco rapido** su una cella segna o toglie una nota (candidato) per il personaggio selezionato, senza confermarlo; **tenere premuto fermo** su una cella per circa mezzo secondo conferma lì il personaggio — e cancella automaticamente tutte le sue note esistenti su tutta la griglia, dato che il dubbio è ormai risolto; **tenere premuto e trascinare** su più celle applica lo strumento a tutte in un solo passaggio (utile anche per segnare X o cancellare più celle di fila) invece di ripetere il gesto una per una. Da tastiera: Invio/Spazio equivalgono al clic (nota), Maiusc+Invio equivale al tenere premuto (piazza). Confermare un personaggio segna automaticamente una X sul resto della sua riga/colonna, dato che nessun altro può stare lì. Tenere premuto di nuovo sulla cella in cui hai già confermato quel personaggio lo rimuove (e ritira anche le X automatiche insieme a lui); spostarlo direttamente su un'altra cella libera fa lo stesso con la vecchia posizione. Un cronometro in alto mostra il tempo trascorso, e si ferma quando risolvi il caso. **Pulisci tutto** richiede di **tenere premuto** il pulsante, per evitare di cancellare tutto per sbaglio. Usa **Annulla**, poi **Verifica soluzione**: dice solo se il caso è completamente risolto o no, senza rivelare quante posizioni sono già giuste (altrimenti si potrebbe indovinare per tentativi invece che dedurre dagli indizi). Man mano che confermi un personaggio sulla griglia, la sua scheda nel pannello indizi si affievolisce, come un nome depennato dalla lista dei sospettati; passando il mouse (o il focus) sul suo chip nella toolbar, o su una singola riga di indizio nella sua scheda, si evidenziano temporaneamente sulla griglia le caselle a cui quell'indizio fa riferimento; cliccando il chip la scheda si evidenzia invece in modo stabile con un anello del suo colore, per trovare subito tutti i suoi indizi.
...
### Modalità campagna
Dalla home, **🕵️ Modalità Campagna** porta a una sequenza di casi giocati in ordine: risolvere un caso sblocca il successivo. Richiede l'accesso con un account Google (solo per questa modalità — editor e gioco libero restano senza login), e sincronizza i progressi (piazzamenti, note, segni X, casi completati) su Firestore: puoi lasciare un caso a metà su un dispositivo e ritrovarlo identico accedendo con lo stesso account da un altro. Un caso già completato resta comunque rigiocabile/ripulibile, come nel gioco libero. Un link diretto a un caso specifico (`campaign.html?campaign=<id>&case=<id>`) funziona anche prima di aver fatto login: chiede l'accesso e poi apre subito quel caso.

Le campagne si creano solo dall'apposito **editor campagne** (`campaign-editor.html`), riservato al proprietario del sito: vi si crea una campagna (titolo, descrizione) e vi si aggiungono i casi incollando l'URL di un `.murdoku.json` già pubblicato (lo stesso meccanismo usato per condividere un caso singolo via link — vedi sopra), riordinandoli con ↑/↓.
